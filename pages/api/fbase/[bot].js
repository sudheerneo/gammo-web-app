...
const handler = async (req, res) => {
  ...
  try {
    ...
    //firestore get
      ...
      const lastsuccess = {
        ...
        payment: {
          ...
          payment_request:
            "https://test.instamojo.com/api/1.1/payment-requests/39b8e3a1293d431d91f191df9767a4b2/",
      ...
      vals
        ? refInsert.update(
            { "donations.lastsuccess": lastsuccess },
            { merge: false }
          ) &&
          refInsert.set(
            {
    ...
    //firestore post
      ...
      const user = db.collection("usersdata").doc(session.user.email);

      const doc = await user.set({ GP: 50, lastGp: 70 }, { merge: false });
